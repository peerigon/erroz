...
- **Source**: `src/` - All source code and tests
  - `main.ts` - Public API: the `erroz` factory plus `AbstractError` and shared types
  - `AbstractError.ts` - Base `Error` subclass that captures the stack trace
  - `defaultRenderer.ts` - Default `%variable` template renderer
  - `lib.ts` - Helpers (e.g. deriving JSend status from a status code)
- **Tests**: Co-located with source files using `.test.ts` suffix (`src/main.test.ts`)
...
- `erroz` is a factory that returns a custom `Error` subclass for each error definition
- Errors support metadata, templated messages, stack traces and JSend serialization
